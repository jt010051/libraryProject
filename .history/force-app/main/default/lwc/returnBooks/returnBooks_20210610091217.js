...
export default class BookList extends LightningElement {
    ...
    getSelectedName(event) {
        ...
        this.selectedRowsId.forEach(id => {
            if(!this.tableData.some(data => data.id === id))
                selectedRows.push(this.orders.find(data => data.id === id));
        });
        // Keep track of selected rows
        selectedRows.forEach(row => {
            if(!this.selectedRowsId.includes(row.id)) {
